...
def main(batch_size: int, org_id: str | None) -> None:
    ...
    stats = repo.backfill_ad_facts(org_id=org_id, batch_size=batch_size)
    print(f"Backfill complete: {stats}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Backfill ad_facts for existing ads.")
    parser.add_argument("--batch-size", type=int, default=500, help="Batch size for processing ads without facts.")
