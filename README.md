...
- Ubuntu bootstrap: `./scripts/setup-ubuntu-dev.sh` installs the required local tooling for the existing dev scripts on Ubuntu 22.04/24.04 (Python 3.11, Node.js/npm, Docker Compose, ngrok), bootstraps backend/frontend/shopify dependencies, and then validates that your real env files exist. It does **not** generate placeholder `.env` files or start services.
- Required local config before launch:
  - backend env in one of `mos/backend/.env`, `.env`, or `.env.local.consolidated` (use `mos/backend/.env.example` as the key reference)
  - frontend env at `mos/frontend/.env.local` with at least `VITE_CLERK_PUBLISHABLE_KEY`, `VITE_API_BASE_URL`, and `VITE_CLERK_JWT_TEMPLATE`
  - Shopify bridge env at `shopify-funnel-app/.env` (use `shopify-funnel-app/.env.example` as the key reference)
  - Shopify manifest at `shopify-funnel-app/shopify.app.toml`
- Infra: `cd mos/infra && docker compose up -d` starts the base Postgres/Temporal stack (Postgres on 5433, Temporal on 7234, Temporal UI on 8234). Temporal DB name is `temporal`. Postiz uses its own sidecar stack and should be started with `./scripts/start-postiz.sh` or the canonical `./scripts/open-dev-terminals.sh` launcher.
...
- Services: launch all local services with `./scripts/open-dev-terminals.sh`. It starts Shopify funnel, ngrok, Temporal, Postiz, backend, worker, frontend, and Medusa. Backend `./scripts/start-backend.sh` and Temporal worker `python -m app.temporal.worker` both run against Temporal at `localhost:7234`. On the shared private VM, `start-backend.sh` binds to the NetBird `wt0` address when present and otherwise falls back to `127.0.0.1`; use `./scripts/resolve-dev-access-url.sh 8008` to print the canonical reachable backend URL. Smoke workflow `TestCampaignWorkflow` executed end-to-end (seed org 23c24cde-9627-4b19-b308-81d612b41eac, client 9d0fb398-0133-47f3-9d19-ca6454ca2af6).
- Frontend: `cd mos/frontend && npm install` then start via `./scripts/start-frontend.sh` (preferred). If you run Vite manually, bind it to the NetBird `wt0` address, `localhost`, or `127.0.0.1`, never `0.0.0.0` on the shared VM. Use `./scripts/resolve-dev-access-url.sh 5275` to print the canonical reachable UI URL. Clerk publishable key stays in `.env.local`, and `VITE_API_BASE_URL` should point at the matching backend URL. After signing in you can: create clients and start onboarding; create campaigns and start planning; view workflow runs, refresh, and send approval/stop signals from the Workflows tab.
...
- CORS: configure `BACKEND_CORS_ORIGINS` (JSON array) in `mos/backend/.env` to match allowed frontend origins (for example `["https://moshq.app","http://127.0.0.1:5173"]`).
