    ...
    def mirror_asset(self, media: MediaAsset) -> MediaAsset:
        ...
        try:
            ...
            if existing and existing.storage_key:
                media.sha256 = existing.sha256
                media.storage_key = existing.storage_key
                media.preview_storage_key = media.preview_storage_key or existing.preview_storage_key
                media.bucket = media.bucket or existing.bucket
                media.preview_bucket = media.preview_bucket or existing.preview_bucket
                media.mime_type = media.mime_type or existing.mime_type or mime
                media.size_bytes = media.size_bytes or existing.size_bytes or download.size_bytes
                media.width = media.width or existing.width
                media.height = media.height or existing.height
                media.mirror_status = existing.mirror_status or MediaMirrorStatusEnum.succeeded
                media.mirror_error = existing.mirror_error
                media.mirrored_at = existing.mirrored_at or now
                self.session.add(media)
                self.session.flush()
                return media
            ...
            if existing:
                # Re-point ad links to the canonical media when safe; drop redundant links.
                # Copy links to the canonical media (no-op if already linked), then remove dup links and asset.
                link_rows = list(
                ...
                # Drop the duplicate media row to avoid sha collision; keep existing as canonical.
