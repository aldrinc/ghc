...
class FunnelPagesRepository:
    ...
    def create(self, *, funnel_id: str, name: str, slug: str, ordering: int) -> FunnelPage:
        page = FunnelPage(funnel_id=funnel_id, name=name, slug=slug, ordering=ordering)
...
class FunnelPublicRepository:
    ...
    def get_asset_by_public_id(self, *, public_id: str) -> Optional[FunnelAsset]:
        stmt = select(FunnelAsset).where(FunnelAsset.public_id == public_id)
        return self.session.scalars(stmt).first()
