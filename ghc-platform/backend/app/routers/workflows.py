...
async def get_workflow_run(
    ...
    if run.client_id:
        client_canon = artifacts_repo.get_latest_by_type(
            org_id=auth.org_id, client_id=run.client_id, artifact_type=ArtifactTypeEnum.client_canon
        )
        metric_schema = artifacts_repo.get_latest_by_type(
            org_id=auth.org_id, client_id=run.client_id, artifact_type=ArtifactTypeEnum.metric_schema
        )
    if run.campaign_id:
...
async def approve_assets(
    ...
    await handle.signal(
        "approve_assets",
        {"approved_ids": body.get("approved_ids", []), "rejected_ids": body.get("rejected_ids", [])},
    )
    repo.log_activity(
        workflow_run_id=workflow_run_id,
        step="approve_assets",
