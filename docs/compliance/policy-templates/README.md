# Compliance Policy Template Markdown Files

Exported from `mos/backend/app/services/compliance.py` (`_POLICY_TEMPLATES`).
...
`contact_support` is theme-managed through `templates/page.contact*.json` and is not sourced from a markdown file.
