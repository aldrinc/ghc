...
- Shopify bridge integration (required for Shopify connection/product mapping/checkout): `SHOPIFY_APP_BASE_URL`, `SHOPIFY_INTERNAL_API_TOKEN`, optional `SHOPIFY_ORDER_WEBHOOK_SECRET`.
  - `SHOPIFY_APP_BASE_URL` should point to a bridge URL that the `mos/backend` runtime can actually reach.
  - For colocated production installs, prefer the local bridge listener (for example `http://127.0.0.1:8011`) instead of routing server-to-server checkout traffic through the public app hostname.
...
- Frontend: set `VITE_CLERK_PUBLISHABLE_KEY`, `VITE_API_BASE_URL` (point to deployed backend), and `VITE_CLERK_JWT_TEMPLATE` (defaults to `backend`).
  - If you expose the backend on a separate HTTPS origin such as `https://api.moshq.app`, nginx must serve that exact hostname with its own valid `443` certificate. Otherwise the browser will fail before CORS/auth logic runs.
...
- CI/CD: `.github/workflows/docker-images.yml` runs backend pytest + alembic against Postgres, builds the frontend, then builds/pushes images to GHCR (main only). Its `deploy` job is now reserved for manual `workflow_dispatch` runs; it SSHes to a target host and runs `docker compose -f mos/infra/docker-compose.deploy.yml up -d`, and requires `DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_SSH_KEY`, `DEPLOY_PATH`, `GHCR_USERNAME`, and `GHCR_TOKEN` secrets plus a `.env.production` file on the host.
- The manual Docker deploy now also installs the repo-managed `moshq.app` nginx site template from `ops/moshq-app/app/nginx.conf`, proxying `/` to the frontend container on `127.0.0.1:8080` and `/api/` to the backend on `127.0.0.1:8008`. This keeps SPA deep links on the frontend nginx path instead of any legacy static server.
- Self-deploy guard: `.github/workflows/self-deploy.yml` is the automatic production publish path for `main`. It triggers from a completed `CI/CD` run on `main`, only proceeds when `ENABLE_PRODUCTION_CD=true`, and fails closed when upstream `CI/CD` is not green.
- Post-push verification: `python3 scripts/check_github_actions.py --sha HEAD --wait --expect-production` polls GitHub Actions for the current commit and exits non-zero unless the real production publish path completed successfully. For `push` runs that means `CI/CD` succeeded and `Self Deploy` applied the plan; for manual `workflow_dispatch` runs it means the `CI/CD` `deploy` job succeeded. Treat a non-zero exit as an incomplete deployment and keep the repair loop open until the script returns `0`.
...
- SPA routing smoke: `python3 scripts/check_spa_routes.py --base-url https://moshq.app --route / --route /sign-in --route /workspaces/overview` must return success so refreshes and pasted deep links resolve to the app shell instead of a static-server 404 page.
