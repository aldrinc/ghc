...
- Self-deploy guard: `.github/workflows/self-deploy.yml` now triggers from a completed `CI/CD` run on `main` instead of directly from `push`. If the upstream `CI/CD` run finishes with anything other than `success`, the workflow fails and blocks production deployment rather than producing a misleading valid run. Manual dispatch remains available for explicit operator-triggered deploys.
- Post-push verification: `python3 scripts/check_github_actions.py --sha HEAD --wait --expect-production` polls GitHub Actions for the current commit and exits non-zero unless the remote CI/CD run and its production deploy job have both completed successfully. Treat a non-zero exit as an incomplete deployment and keep the repair loop open until the script returns `0`.
