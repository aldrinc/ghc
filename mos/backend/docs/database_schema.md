...
Notes:
- `funnels.experiment_spec_id` stores the experiment spec id (string) used to tie funnels to approved experiment angles.
...
erDiagram
    ...
    PRODUCT_BRAND_RELATIONSHIPS {
        ...
        uuid product_id FK
        uuid brand_id FK
        product_brand_relationship_type relationship_type
        product_brand_relationship_source source_type
        text source_id
    ...
    ORGS ||--o{ PRODUCT_BRAND_RELATIONSHIPS : org_id
    BRANDS ||--o{ BRAND_CHANNEL_IDENTITIES : brand_id
    BRANDS ||--o{ PRODUCT_BRAND_RELATIONSHIPS : brand_id
    CLIENTS ||--o{ PRODUCT_BRAND_RELATIONSHIPS : client_id
    PRODUCTS ||--o{ PRODUCT_BRAND_RELATIONSHIPS : product_id
    USERS ||--o{ PRODUCT_BRAND_RELATIONSHIPS : created_by_user_id
...
- `product_brand_relationship_type`: competitor
- `product_brand_relationship_source`: onboarding_seed, competitor_discovery, ads_ingestion, manual_admin
...
| relationship_type | enum(product_brand_relationship_type) | no |  |
| source_type | enum(product_brand_relationship_source) | no |  |
| source_id | text | yes |  |
...
| created_at | timestamptz | no | server default `now()` |
| updated_at | timestamptz | no | server default `now()` |
| Unique |  |  | `(org_id, idea_workspace_id, product_id, doc_key, sha256)` |
