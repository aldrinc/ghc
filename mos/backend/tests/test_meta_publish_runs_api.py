...
import app.services.meta_ads as meta_ads_service
...
from app.services.meta_ads import MetaAdsClient, MetaAdsError, MetaAdsRateLimitError
...
def test_publish_meta_run_creates_paused_entities_and_history(api_client, db_session, monkeypatch) -> None:
    ...
    assert history_payload[0]["items"] == []

    detail_response = api_client.get(
        f"/meta/campaigns/{campaign_id}/publish-runs/{publish_payload['id']}"
    )
...
def test_publish_meta_run_resumes_partial_failures_without_creating_a_new_meta_campaign(
    api_client, db_session, monkeypatch
) -> None:
    client_id, product_id, campaign_id = _create_campaign_with_product(
        api_client,
        suffix="publish-resume-partial-failure",
        db_session=db_session,
    )
    funnel_id = str(uuid4())
    brief_id = "brief-publish-resume-partial-failure"
    _create_funnel_scoped_brief(
        db_session,
        client_id=client_id,
        campaign_id=campaign_id,
    ...
    for index in range(2):
        asset = _create_asset(
            ...
            campaign_id=campaign_id,
            batch_id="latest-run",
            suffix=f"publish-resume-partial-failure-{index}",
            asset_brief_id=brief_id,
        )
        expected_storage_keys.add(str(asset.storage_key))
        _create_meta_publish_inputs(
            ...
            asset=asset,
            campaign_id=campaign_id,
            experiment_key=f"exp-publish-resume-partial-failure-{index}",
            with_targeting=True,
        )
    ...
    class _FakeMetaClient:
        def upload_image(self, **_kwargs):
            ...
            return {"images": {"creative.jpg": {"hash": f"hash_{counters['upload_image']}"}}}

        def create_campaign(self, **_kwargs):
            counters["create_campaign"] += 1
            return {"id": "meta_campaign_resume_partial", "status": "PAUSED"}

        def create_adset(self, **kwargs):
            counters["create_adset"] += 1
            bucket_suffix = kwargs["payload"]["name"].split()[-1]
            return {"id": f"meta_adset_resume_partial_{bucket_suffix}", "status": "PAUSED"}

        def create_adcreative(self, **_kwargs):
            counters["create_adcreative"] += 1
            return {"id": f"meta_creative_resume_partial_{counters['create_adcreative']}"}

        def create_ad(self, **_kwargs):
            counters["create_ad"] += 1
            if counters["create_ad"] == 2:
                raise MetaAdsError("Meta Graph API request failed: The read operation timed out")
            return {"id": f"meta_ad_resume_partial_{counters['create_ad']}", "status": "PAUSED"}
    ...
    assert first_payload["status"] == "partial_failed"
    assert counters["create_campaign"] == 1
    assert counters["create_adset"] == DEFAULT_META_PUBLISH_BUCKET_COUNT
    assert counters["upload_image"] == 2
    ...
    second_response = api_client.post(
        ...
        json={
            ...
            "funnelId": funnel_id,
            "publishBaseUrl": "https://shop.thehonestherbalist.com",
            "campaignName": "Honest Herbalist Launch Retry",
            "campaignObjective": "OUTCOME_SALES",
        },
    )
    ...
    assert second_payload["metaCampaignId"] == "meta_campaign_resume_partial"
    assert second_payload["id"] != first_payload["id"]
    assert second_payload["metadata"]["resumeFromRunId"] == first_payload["id"]
    assert counters["create_campaign"] == 1
    assert counters["create_adset"] == DEFAULT_META_PUBLISH_BUCKET_COUNT
    assert counters["upload_image"] == 2
    assert counters["create_adcreative"] == 2
    assert counters["create_ad"] == 3
...
def test_publish_meta_run_retries_retryable_meta_ad_create_failures(
    api_client, db_session, monkeypatch
) -> None:
    client_id, product_id, campaign_id = _create_campaign_with_product(
        api_client,
        suffix="publish-ad-retry",
        db_session=db_session,
    )
    funnel_id = str(uuid4())
    brief_id = "brief-publish-ad-retry"
    _create_funnel_scoped_brief(
        db_session,
        client_id=client_id,
        campaign_id=campaign_id,
    ...
    _create_meta_publish_inputs(
        ...
        asset=asset,
        campaign_id=campaign_id,
        experiment_key="exp-publish-ad-retry",
        with_targeting=True,
    )
    _upsert_meta_profile(api_client, client_id=client_id)
    ...
    class _FakeStorage:
        def download_bytes(self, *, key: str, bucket: str | None = None) -> tuple[bytes, str]:
            _ = bucket
            assert key == "creative/publish-ad-retry.jpg"
            return content, "image/jpeg"

    class _FakeMetaClient:
        def upload_image(self, **kwargs):
            assert kwargs["ad_account_id"] == "act_123456"
            return {"images": {kwargs["filename"]: {"hash": "hash_ad_retry"}}}

        def create_campaign(self, **_kwargs):
            return {"id": "meta_campaign_ad_retry", "status": "PAUSED"}

        def create_adset(self, **kwargs):
            bucket_suffix = kwargs["payload"]["name"].split()[-1]
            return {"id": f"meta_adset_ad_retry_{bucket_suffix}", "status": "PAUSED"}

        def create_adcreative(self, **_kwargs):
            return {"id": "meta_creative_ad_retry"}

        def create_ad(self, **_kwargs):
            ad_counter["count"] += 1
            if ad_counter["count"] == 1:
                raise MetaAdsError(
                    "Meta Graph API error (500).",
                    status_code=500,
                    error_payload={
                        "error": {
            ...
            return {"id": f"meta_ad_ad_retry_{ad_counter['count']}", "status": "PAUSED"}
    ...
    assert payload["status"] == "published"
    assert payload["items"][0]["status"] == "published"
    assert payload["items"][0]["metaAdId"] == "meta_ad_ad_retry_2"
    assert ad_counter["count"] == 2
...
def test_update_meta_adset_clears_bid_cap_in_place(api_client, db_session, monkeypatch) -> None:
    ...
    class _FakeMetaClient:
        def update_adset(self, *, adset_id: str, payload: dict[str, object], throttle_scope: str | None = None):
            _ = throttle_scope
...
def test_meta_management_apply_mode_persists_artifacts(api_client, db_session, monkeypatch) -> None:
    ...
    class _FakeManagementClient:
        def get_object(self, *, object_id: str, fields: str, throttle_scope: str | None = None):
            _ = fields
            _ = throttle_scope
        ...
        def update_ad(self, *, ad_id: str, payload: dict[str, object], throttle_scope: str | None = None):
            _ = throttle_scope
...
def test_meta_management_plan_only_uses_cached_snapshot_until_refresh(
    api_client, db_session, monkeypatch
) -> None:
    client_id, product_id, campaign_id = _create_campaign_with_product(
        api_client,
        suffix="management-plan-cache",
        db_session=db_session,
    )
    _ = product_id
    workspace_config = _seed_meta_workspace_config(db_session, client_id=client_id)
    ...
    local_meta_campaign = MetaCampaign(
        ...
        campaign_id=campaign.id,
        ad_account_id="act_123456",
        request_id="meta-launch-plan:test:management-cache",
        meta_campaign_id="meta_campaign_cache_123",
        name="Managed Campaign",
        objective="OUTCOME_SALES",
        status="PAUSED",
        metadata_json={},
    ...
    def _plan(generated_at: str) -> MetaManagementPlan:
        return MetaManagementPlan(
            mode="plan_only",
            generatedAt=generated_at,
            window={"datePreset": "last_3d"},
            campaign={"id": "meta_campaign_cache_123", "status": "PAUSED"},
            adsets=[],
            objectState=MetaObjectStateSummary(
                deliveryState="paused",
                deliverySummary="Paused in Meta.",
                insightsRowCount=0,
            ),
            rows=[],
            actions=[],
            warnings=["no_delivery_rows.paused"],
            benchmarkStatus=MetaManagementBenchmarkStatus(
                requestedMode="best_effort",
                available=False,
                reasonCode="unsupported_delivery_mode",
                reason="External delivery campaigns do not expose funnel benchmarks.",
            ),
        )

    first_generated_at = datetime.now(timezone.utc).isoformat()
    monkeypatch.setattr(
        meta_ads_router,
        "execute_meta_management_plan",
        lambda **_kwargs: SimpleNamespace(
            campaign=campaign,
            plan=_plan(first_generated_at),
    ...
    first_response = api_client.post(
        "/meta/management/plan",
        json={
            "metaCampaignId": "meta_campaign_cache_123",
            "clientId": client_id,
            "metaConfigId": str(workspace_config.id),
            "mode": "plan_only",
        },
    )
    ...
    monkeypatch.setattr(meta_ads_router, "execute_meta_management_plan", _should_not_execute)

    cached_response = api_client.post(
        "/meta/management/plan",
        json={
            "metaCampaignId": "meta_campaign_cache_123",
            "clientId": client_id,
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: SimpleNamespace(
            campaign=campaign,
            plan=_plan(refreshed_generated_at),
            resolved=SimpleNamespace(),
            ad_account_id="act_123456",
        ),
    )
    ...
    assert refresh_payload["generatedAt"] == refreshed_generated_at
...
def test_fetch_ad_level_insights_normalizes_ad_account_id() -> None:
    ...
    class _FakeClient:
        def _request(self, method, path, *, params=None, throttle_scope=None):
            _ = method
            _ = params
            captured_paths.append(path)
            captured_scopes.append(throttle_scope)
            return {"data": [], "paging": {}}
    ...
    assert captured_scopes == ["act_123456"]


def test_meta_ads_client_retries_rate_limited_reads(monkeypatch) -> None:
    class _FakeResponse:
    ...
    responses = [
        _FakeResponse(
            status_code=429,
            payload={"error": {"code": 17, "message": "Ad Account Has Too Many API Calls"}},
            headers={"Retry-After": "1"},
        ),
        _FakeResponse(
            status_code=200,
            payload={"id": "act_123456"},
        ),
    ]
    request_paths: list[str] = []
    sleep_calls: list[float] = []

    def _fake_request(method, url, *, params=None, data=None, files=None, timeout=None):
        _ = method
        _ = params
        _ = data
        _ = files
        _ = timeout
        request_paths.append(url)
        return responses.pop(0)

    monkeypatch.setattr("app.services.meta_ads.httpx.request", _fake_request)
    monkeypatch.setattr("app.services.meta_ads.time.sleep", lambda seconds: sleep_calls.append(seconds))
    monkeypatch.setattr("app.services.meta_ads.settings.META_GRAPH_RATE_LIMIT_MAX_RETRIES", 1)
    ...
    payload = client.get_object(object_id="act_123456", fields="id")

    assert payload == {"id": "act_123456"}
    assert len(request_paths) == 2
    assert sleep_calls == [1.0]


def test_meta_ads_client_defers_when_account_read_slots_are_exhausted(monkeypatch) -> None:
    throttle_scope = "act_654321"
    state = meta_ads_service._read_pressure_state_for(throttle_scope)
    with state.condition:
        state.active_read_requests = 1
        state.cooldown_until_monotonic = 0.0

    monkeypatch.setattr("app.services.meta_ads.settings.META_GRAPH_MAX_CONCURRENT_READS_PER_ACCOUNT", 1)
    monkeypatch.setattr("app.services.meta_ads.settings.META_GRAPH_READ_SLOT_WAIT_TIMEOUT_SECONDS", 0.0)
    monkeypatch.setattr(
        "app.services.meta_ads.httpx.request",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("Deferred reads must not hit Meta.")),
    )
    ...
    try:
        with pytest.raises(MetaAdsRateLimitError) as exc_info:
            client.get_object(object_id=throttle_scope, fields="id")
    finally:
        with state.condition:
    ...
    assert exc_info.value.ad_account_id == throttle_scope
    assert exc_info.value.deferred is True


def test_build_management_plan_skips_insights_for_paused_campaigns(monkeypatch) -> None:
    monkeypatch.setattr(
        "app.services.meta_media_buying.fetch_meta_campaign_snapshot",
        lambda **_kwargs: (
            {"id": "meta_campaign_paused_123", "status": "PAUSED", "effective_status": "PAUSED"},
            [],
            [{"id": "meta_ad_123", "status": "PAUSED", "effective_status": "PAUSED"}],
        ),
    )

    def _unexpected_insights(**_kwargs):
        raise AssertionError("Paused campaign should not fetch ad-level insights.")

    monkeypatch.setattr("app.services.meta_media_buying.fetch_ad_level_insights", _unexpected_insights)

    plan = build_management_plan(
        client=SimpleNamespace(),
        ad_account_id="act_123456",
        campaign_id="meta_campaign_paused_123",
        mode="plan_only",
        insights=MetaInsightsConfig(datePreset="last_3d"),
        cut_rules=MetaCutRuleConfig(),
        event_mappings=MetaEventMappings(),
    )

    assert plan.rows == []
    assert plan.objectState.deliveryState == "paused"
...
def test_meta_management_plan_returns_clean_rate_limit_error(
    api_client, db_session, monkeypatch
) -> None:
    client_id, product_id, campaign_id = _create_campaign_with_product(
        api_client,
        suffix="management-rate-limit",
        db_session=db_session,
    )
    _ = product_id
    _seed_meta_workspace_config(db_session, client_id=client_id)
    ...
    local_meta_campaign = MetaCampaign(
        ...
        campaign_id=campaign.id,
        ad_account_id="act_123456",
        request_id="meta-launch-plan:test:rate-limit",
        meta_campaign_id="meta_campaign_rate_limit_123",
    ...
    response = api_client.post(
        "/meta/management/plan",
        json={
            "metaCampaignId": "meta_campaign_rate_limit_123",
            "clientId": client_id,
        },
    )
    ...
    payload = response.json()
    assert payload["detail"]["message"] == "Meta management snapshot deferred due to ad account pressure."
    assert payload["detail"]["meta"]["kind"] == "ad_account_pressure"
    assert payload["detail"]["meta"]["adAccountId"] == "act_123456"
    assert payload["detail"]["meta"]["retryAfterSeconds"] == 7.0
    assert payload["detail"]["meta"]["deferred"] is True
