...
def test_upload_design_system_logo_updates_dark_surface_logo_token(api_client: TestClient, monkeypatch):
    client_resp = api_client.post(
        "/clients", json={"name": "Design System Client", "industry": "SaaS"}
    )
    assert client_resp.status_code == 201
    ...
    def _fake_upload(
        *,
        ...
        asset = Asset(
            ...
            content={},
            public_id=fake_public_id,
            storage_key="assets/fake-dark-logo.png",
            content_type=content_type,
            size_bytes=123,
            width=128,
            height=48,
    ...
    upload_resp = api_client.post(
        f"/design-systems/{design_system_id}/logo?variant=onDark",
        files=[("file", ("logo-dark.png", b"fake-image-bytes", "image/png"))],
    )
    assert upload_resp.status_code == 201
    body = upload_resp.json()
    assert body["publicId"] == str(fake_public_id)
    assert body["variant"] == "onDark"
    assert body["designSystem"]["tokens"]["brand"]["logoOnDarkAssetPublicId"] == str(fake_public_id)
