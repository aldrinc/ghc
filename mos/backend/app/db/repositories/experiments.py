...
from app.db.models import Experiment, Campaign
...
class ExperimentsRepository:
    ...
    def list(
        ...
        if product_id:
            stmt = stmt.join(Campaign, Campaign.id == Experiment.campaign_id).where(
                Campaign.org_id == org_id, Campaign.product_id == product_id
            )
