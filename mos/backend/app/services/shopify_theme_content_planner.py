...
def _build_planner_prompt(
    *,
    ...
    product_payload = {
        ...
        "primaryBenefits": [item.strip() for item in (product.primary_benefits or []) if isinstance(item, str) and item.strip()],
        "featureBullets": [item.strip() for item in (product.feature_bullets or []) if isinstance(item, str) and item.strip()],
        "guaranteeText": str(product.guarantee_text or "").strip(),
        "disclaimers": [item.strip() for item in (product.disclaimers or []) if isinstance(item, str) and item.strip()],
        "offers": [
            {
    ...
    *,
    ...
    for assignment in raw_image_assignments:
        ...
        if not isinstance(path, str) or not path.strip():
            raise ValueError("Theme content planner returned an image assignment with invalid path.")
        if not isinstance(asset_public_id, str) or not asset_public_id.strip():
            raise ValueError(f"Theme content planner returned an image assignment with invalid asset id at {path}.")
        normalized_path = path.strip()
        normalized_asset_public_id = asset_public_id.strip()
        if normalized_path not in image_slot_paths:
            raise ValueError(f"Theme content planner returned an unknown image slot path: {normalized_path}.")
        ...
        if normalized_path in component_image_asset_map:
            raise ValueError(f"Theme content planner returned duplicate image assignment for path {normalized_path}.")
    ...
    if image_slot_paths and set(component_image_asset_map.keys()) != image_slot_paths:
        ...
        raise ValueError(f"Theme content planner did not assign all image slots: {', '.join(missing)}.")
    ...
    for assignment in raw_text_assignments:
        ...
        if slot is None:
            raise ValueError(f"Theme content planner returned an unknown text slot path: {normalized_path}.")
        max_length = slot.get("maxLength")
        if isinstance(max_length, int) and max_length > 0 and len(normalized_value) > max_length:
            raise ValueError(
        ...
        if normalized_path in component_text_values:
            raise ValueError(f"Theme content planner returned duplicate text assignment for path {normalized_path}.")
    ...
    if text_slot_paths and set(component_text_values.keys()) != text_slot_paths:
        ...
        raise ValueError(f"Theme content planner did not assign all text slots: {', '.join(missing)}.")
...
def plan_shopify_theme_component_content(
    *,
    ...
    if not image_slots and not text_slots:
        raise ValueError("No candidate image or text slots were discovered in the Shopify theme templates.")
    ...
    image_assets_payload = [
        {
            ...
            "tags": [tag for tag in (asset.tags or []) if isinstance(tag, str) and tag.strip()][:20],
