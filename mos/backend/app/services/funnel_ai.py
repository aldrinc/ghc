...
def _collect_image_plans(
    *,
    ...
    def _with_image_target_details(exc: AiAttachmentError, *, path: str, obj: dict[str, Any], asset_key: str) -> AiAttachmentError:
        image_source = obj.get("imageSource")
        if image_source is None:
            image_source = obj.get("image_source")
        reference_public_id = obj.get("referenceAssetPublicId")
        if reference_public_id is None:
            reference_public_id = obj.get("reference_asset_public_id")
        prompt = obj.get("prompt")
        details: dict[str, Any] = {
            ...
            "hasPrompt": isinstance(prompt, str) and bool(prompt.strip()),
        }
        if obj.get(asset_key) is not None:
            details[asset_key] = obj.get(asset_key)
        return AiAttachmentError(f"{exc} details={details}")

    def iter_plans(
        root: Any,
        base_path: str,
        *,
        skip_image_components: bool,
        ...
        for path, obj in _walk_json_with_path(root, base_path):
            ...
            if not skip_image_components and obj.get("type") == "Image":
                ...
                if isinstance(props, dict):
                    ...
                    normalized = _normalize_reference_driven_image_source(props)
                    if normalized and context:
                        context.dirty = True
                    props_path = f"{path}.props"
                    try:
                        target = _extract_image_prompt_target(props, "assetPublicId")
                    except AiAttachmentError as exc:
                        raise _with_image_target_details(
                            exc,
                            path=props_path,
                            obj=props,
            ...
            if not asset_key:
                continue
            normalized = _normalize_reference_driven_image_source(obj)
            if normalized and context:
                context.dirty = True
            try:
                target = _extract_image_prompt_target(obj, asset_key)
            except AiAttachmentError as exc:
                raise _with_image_target_details(exc, path=path, obj=obj, asset_key=asset_key) from exc
            if target:
                prompt, reference_public_id, image_source, aspect_ratio = target
    ...
    for ctx in config_contexts:
        iter_plans(
            ctx.parsed,
            f"{ctx.component_type}.{ctx.key}",
            skip_image_components=True,
            context=ctx,
        )
    return plans
...
def _normalize_reference_driven_image_source(obj: dict[str, Any]) -> bool:
    """Unsplash does not support referenceAssetPublicId; drop it and keep unsplash routing."""
    reference_public_id_raw = obj.get("referenceAssetPublicId") or obj.get("reference_asset_public_id")
    if not isinstance(reference_public_id_raw, str) or not reference_public_id_raw.strip():
        return False

    image_source_raw = obj.get("imageSource")
    if image_source_raw is None:
        image_source_raw = obj.get("image_source")
    if not isinstance(image_source_raw, str) or not image_source_raw.strip():
        return False
    if image_source_raw.strip().lower() != "unsplash":
        return False

    changed = False
    if "referenceAssetPublicId" in obj:
...
def _extract_image_prompt_target(
    ...
    if image_source == "unsplash" and reference_public_id:
        raise AiAttachmentError(
            "Unsplash images do not support referenceAssetPublicId. "
            "Set imageSource='ai' or remove referenceAssetPublicId."
        )
    ...
    *,
    ...
    if template_mode:
        template_image_guidance = (
            ...
            "- If referenceAssetPublicId is set: keep assetPublicId empty, set imageSource='ai', and include a prompt that preserves the same product identity.\n\n"
        )
        if template_kind == "sales-pdp":
            template_image_guidance += (
    ...
    if template_mode and template_kind == "sales-pdp":
        template_config_guidance = (
            ...
            + (
                ...
                "- In imported HTML template mode, SalesPdpComparison.config MUST NOT use legacy keys badge/title/swipeHint/columns.pup/columns.disposable/pup/disposable rows.\n"
    ...
    system = {
        ...
        "content": (
            ...
            "   - If referenceAssetPublicId is set: include prompt, leave assetPublicId empty, set imageSource='ai', and keep the same product identity as the reference image\n"
