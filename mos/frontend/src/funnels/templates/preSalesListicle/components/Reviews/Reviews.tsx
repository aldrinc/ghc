...
export function Reviews({ reviews, copy, starsAriaLabelTemplate }: Props) {
  ...
  if (!Array.isArray(active.images) || active.images.length === 0) {
    throw new Error(
      "PreSalesReviews requires slides[].images to be a non-empty array. Regenerate the pre-sales page config with images[] on each slide."
    )
  }
        ...
        <div className={styles.carousel}>
          ...
          <div key={`${index}-${direction}`} className={`${styles.grid} ${direction === 'next' ? styles.enterNext : styles.enterPrev}`}>
            ...
            <div className={styles.media}>
              {main ? (
                <img className={styles.mediaMain} src={resolveImageSrc(main)} alt={main.alt} loading="lazy" decoding="async" />
              ) : (
