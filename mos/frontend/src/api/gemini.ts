...
import type { GeminiChatRequestPayload, GeminiContextFile, GeminiStreamEvent } from "@/types/gemini";
...
export function useGeminiContext(params: {
  ...
  return useQuery<{ files: GeminiContextFile[] }>({
    queryKey: ["gemini-context", params.ideaWorkspaceId, params.clientId, params.productId, params.campaignId],
    queryFn: () => {
      ...
      return get(`/gemini/context?${search.toString()}`);
    },
    enabled: Boolean(params.ideaWorkspaceId),
  });
}

export type GeminiStreamHandle = { abort: () => void };
...
export function useGeminiStream() {
  ...
  return useCallback(
    async (
      payload: GeminiChatRequestPayload,
      ...
      const resp = await fetch(`${defaultBaseUrl}/gemini/chat/stream`, {
        method: "POST",
        headers,
        ...
        try {
          while (true) {
            ...
            while (boundary !== -1) {
              ...
              try {
                const parsed = JSON.parse(payloadStr) as GeminiStreamEvent;
                if (parsed.type === "done") {
                  sawDone = true;
                }
                onEvent(parsed);
                ...
                console.error("Failed to parse Gemini stream chunk", err, raw);
              }
              boundary = buffer.indexOf("\n\n");
            }
          }
