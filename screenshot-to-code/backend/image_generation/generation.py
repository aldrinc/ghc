...
from image_generation.gemini import GEMINI_IMAGE_MODEL, generate_image_gemini
...
async def process_tasks(
    ...
    api_key: str,
    base_url: str | None,
    model: Literal["dalle3", "flux", "gemini-2.5-flash-image"],
) -> List[Union[str, None]]:
    ...
    elif model == GEMINI_IMAGE_MODEL:
        tasks = [generate_image_gemini(prompt, api_key) for prompt in prompts]
        results = await asyncio.gather(*tasks, return_exceptions=True)
    else:
        results = []
