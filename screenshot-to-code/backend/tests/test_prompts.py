...
class TestCreatePrompt:
    ...
    def test_plan_update_with_file_snapshot_prefers_snapshot_over_history(self) -> None:
        plan = derive_prompt_construction_plan(
            stack=self.TEST_STACK,
            input_mode="image",
            generation_type="update",
    ...
    @pytest.mark.asyncio
    async def test_image_mode_create_includes_template_output_policy(self) -> None:
        with patch("prompts.system_prompt.SYSTEM_PROMPT", new=self.MOCK_SYSTEM_PROMPT):
            messages = await build_prompt_messages(
                stack=self.TEST_STACK,
                input_mode="image",
        ...
        user_text = text_part.get("text")
        assert isinstance(user_text, str)
        assert "Template output requirement:" in user_text
        assert "Centralize theme values" in user_text
        ...
        with patch(
            ...
            expected: ExpectedResult = {
                "messages": [
                    ...
                    {
                        "role": "user",
                        "content": "<CONTAINS:Make the background blue>",
                    },
                    {
                        "role": "assistant",
        ...
        with patch(
            ...
            expected: ExpectedResult = {
                "messages": [
                    ...
                    {
                        "role": "user",
                        "content": "<CONTAINS:Add a sidebar>",
                    },
                    {
                        "role": "assistant",
