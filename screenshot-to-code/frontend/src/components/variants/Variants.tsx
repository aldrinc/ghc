...
function Variants() {
  ...
  const continuableVariantIndex =
    findContinuationCandidate(commit, commits)?.issueIndex ?? -1;
  ...
  }, [variants.length, isCommitted, handleVariantClick]);
    ...
    <div className="pt-2 pb-1">
      <div className="grid grid-cols-2 gap-2">
        {variants.map((variant, index) => {
          const isContinuable = index === continuableVariantIndex;
          let statusColor = "bg-gray-300 dark:bg-gray-600";
          if (variant.status === "complete") statusColor = "bg-green-500";
          else if (variant.status === "paused" || isContinuable) statusColor = "bg-amber-500";
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center px-2 py-1 bg-white dark:bg-zinc-900">
                ...
                {isContinuable ? (
                  <span className="ml-auto shrink-0 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-medium text-amber-800 dark:bg-amber-900/30 dark:text-amber-200">
                    Continue
                  </span>
