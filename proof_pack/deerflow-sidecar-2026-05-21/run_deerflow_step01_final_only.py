...
EVENTS_PATH = OUTPUT_DIR / "deerflow-step01-final-only-events.jsonl"
FINAL_PATH = OUTPUT_DIR / "deerflow-step01-final-only.md"
META_PATH = OUTPUT_DIR / "deerflow-step01-final-only-meta.json"
...
- Do not use bash.
- Do not call write_file. Return the final report in the chat only. The wrapper will persist it.

Bounds for this smoke:
- Use at most 2 web_search calls.
- Use at most 2 web_fetch calls.
...
4. Short production-readiness note: where DeerFlow+DeepSeek is stronger, weaker, and what wrapper is needed.
"""
...
def _timeout_handler(_signum, _frame):
    raise RunTimedOut("DeerFlow final-only smoke timed out")
...
def main() -> int:
    ...
    started = time.time()
    thread_id = "marketi-step01-final-only-2026-05-21"
    status = "unknown"
    error = None
    ...
    try:
        with EVENTS_PATH.open("a", encoding="utf-8") as events_file:
            for event in client.stream(PROMPT, thread_id=thread_id, recursion_limit=50):
