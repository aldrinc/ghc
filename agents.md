...
No standalone/static HTML funnel deploys outside `html-deploy-v1`. Do not fall back to legacy production deployment strategies. Do not use legacy standalone flows, ad hoc route-scoped artifact copies, runtime-bundle fallback for HTML funnels, or manual server-side HTML replacements for production HTML funnel deployment unless I explicitly authorize a break-glass override in the current thread.
